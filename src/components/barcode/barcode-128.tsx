 ...
 *   fija un mínimo, pero por debajo de ~1.5px en pantalla (≈0.2mm impreso) el
 *   contraste se degrada y aumentan las fallas de lectura; para etiquetas de
 *   tubo (espacio reducido) usar 1.3–1.5, no menos.
...
export function Barcode128({
  ...
  for (const s of stripes) {
    if (s.bar) {
      rects.push(
        <rect key={k++} x={x * moduleWidth} y={0} width={s.w * moduleWidth} height={height} />
      );
    }
    x += s.w;
  }
